...
use cacheferret::{Error, OutputFormat, Request, run, schema};
...
fn main() -> ExitCode {
    ...
    let Some(value) = cli.value.clone() else {
        let err = Error::Usage {
            message: "no value given (try `cacheferret 21`)".into(),
        };
        emit_error(&err);
